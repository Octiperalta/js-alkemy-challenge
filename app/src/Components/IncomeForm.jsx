...
import { CloseIcon, DollarIcon, LoaderIcon, MoneyIcon } from "../icons";
...
function IncomeForm() {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      await createOperation(description, amount, "income", token);
      history.push("/");
    } catch (err) {
      throw err;
    }
    ...
    <motion.div
      ...
      <div className='px-8 py-8'>
        <div className='bg-gray-50 rounded-lg shadow-md flex flex-col overflow-hidden'>
          <div>
            <div className='flex px-5 py-5 items-center border-b'>
              <div className='bg-indigo-100 rounded-full p-2 '>
                <MoneyIcon className='h-6 w-6 text-indigo-600' />
              </div>
              <h5 className='ml-3 text-xl font-medium text-gray-800'>
              ...
              <form
                ...
                <div className='px-5 py-4 md:flex w-full'>
                  <div className='md:ml-auto flex space-x-3 gap-1 '>
                    ...
                    <button
                      type='submit'
                      className='px-4 py-3 md:py-2 text-md md:text-base flex-grow flex items-center border rounded-md bg-indigo-500 text-white font-medium hover:bg-indigo-600 transition-colors'>
                      {loading ? (
                        <>
